...
void kar( int [ ][ 13 ] );
void dagit( const int [ ][ 13 ], const char *[ ], const char *[ ],char *[][2] );
int isOnePair(char  *playerHand[][2],int cardcount );
int isTwoPair(char  *playerHand[][2],int cardcount );
int isThreeSuits(char  *playerHand[][2],int cardcount );
int isFourSuits(char  *playerHand[][2],int cardcount );
 ...
 int main( )
 {
  ...
  char *playerHand[5][2]={NULL};   //oyuncuya daðýtýlan kartlar tutulacak.
  int deste[4][13] = { 0 };
...
void dagit( const int wDeste[ ][ 13 ], const char *wFaces [ ],const char *wSuits[ ],char *playerHand[][2] )
 {
   int kart, satir, sutun,elKart=0;
   char test[7];
	     ...
	     if ( wDeste[ satir ][ sutun] == kart )
	      {
			...
			playerHand[elKart][0]=wSuits[ satir ];
            playerHand[elKart][1]=wFaces[ sutun ];
            elKart++;
		    if(kart%5==0){
		     	printf("\n_________________");
